      ...
      data: {
        ...
        data: {
          settings: {
            ...
            metaTitle: "Shop Genuine - Best Products",
            metaDescription: "Find the best products at Shop Genuine",
            socialLinks: {
...
// Store Verticals Management
export const storeVerticals = {
  getStoreVerticals: (params: any = {}) => {
    return api.get("/api/admin/store-verticals", { params });
  },
  createStoreVertical: (data: {
    name: string;
    image: File;
    order?: number;
    isActive?: boolean;
    ...
    return api.post("/api/admin/store-verticals", formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
  },
  updateStoreVertical: (
    storeVerticalId: string,
    data: { name?: string; image?: File; order?: number; isActive?: boolean }
  ) => {
    const formData = new FormData();
    if (data.name) formData.append("name", data.name);
    if (data.image) formData.append("image", data.image);
    ...
    if (data.isActive !== undefined)
      formData.append("isActive", String(data.isActive));
    return api.patch(`/api/admin/store-verticals/${storeVerticalId}`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
  },
  deleteStoreVertical: (storeVerticalId: string) => {
    return api.delete(`/api/admin/store-verticals/${storeVerticalId}`);
  },
  toggleStoreVerticalStatus: (storeVerticalId: string) => {
    return api.patch(
      `/api/admin/store-verticals/${storeVerticalId}/toggle-status`
    );
  },
};
