...
export const updateOrderStatus = asyncHandler(async (req, res, next) => {
  ...
  // Push the status change to the customer's devices. Best-effort by design:
  // sendOrderStatusPush never throws, so a notification problem cannot fail an
  // admin's status update or roll back the transaction above.
  await sendOrderStatusPush(order.userId, orderId, order.orderNumber, status);
