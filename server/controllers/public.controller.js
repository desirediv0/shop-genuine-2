...
export const getBrandsByTag = asyncHandler(async (req, res) => {
  const { tag, storeVerticalId } = req.query;

  // Brands are not linked to store verticals directly — a vertical's brands are
  // derived from the products assigned to it, the same way categories are. So
  // "Genuine Nutrition" lists only the brands it actually stocks.
  const verticalProductFilter = storeVerticalId
    ? {
        storeVerticalId,
        isActive: true,
        isDeleted: false,
  ...
  const where = {
    ...(tag ? { tags: { has: tag } } : {}),
    ...(verticalProductFilter ? { products: { some: verticalProductFilter } } : {}),
  };

  const brands = await prisma.brand.findMany({
    where,
    // Only the matching products are needed, and only for the count — pulling
    // every product row of every brand was wasteful even before this filter.
    include: {
      _count: {
        select: {
          products: verticalProductFilter ? { where: verticalProductFilter } : true,
        },
      },
    },
  ...
  res
    .status(200)
    .json(new ApiResponsive(200, { brands: data }, "Brands fetched"));
});
