...
export const checkout = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (storeVerticalId && typeof storeVerticalId === "string") {
      notes.storeVerticalId = storeVerticalId;
    }
...
export const paymentVerification = asyncHandler(async (req, res) => {
  ...
  const {
    ...
    notes,
    storeVerticalId: requestStoreVerticalId,
  } = req.body;
  ...
  try {
    ...
    let couponCode = null;
    let couponId = null;
    let storeVerticalId = requestStoreVerticalId;
      ...
      try {
        ...
        // Fallback to Razorpay order notes
          ...
          if (razorpayOrderDetails.notes) {
            ...
            // Get storeVerticalId from notes if not already provided in request
            if (!storeVerticalId && razorpayOrderDetails.notes.storeVerticalId) {
              storeVerticalId = razorpayOrderDetails.notes.storeVerticalId;
            }
          }
        }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const order = await tx.order.create({
        data: {
          ...
          couponCode,
          couponId: couponId,
          ...(storeVerticalId && typeof storeVerticalId === "string"
            ? { storeVerticalId }
            : {}),
        },
      });
  ...
  const formattedOrders = orders.map((order) => {
    ...
    return {
      ...
      couponDetails: order.coupon
        ...
        : null,
      paymentMethod: order.razorpayPayment?.paymentMethod || order.paymentMethod || "ONLINE",
...
export const cancelOrder = asyncHandler(async (req, res) => {
  ...
  // Confirm the cancellation on the customer's other devices. Never throws.
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        const order = await tx.order.create({
          data: {
            ...
            ...(orderData.storeVerticalId && typeof orderData.storeVerticalId === "string"
              ? { storeVerticalId: orderData.storeVerticalId }
              : {}),
...
export const createCashOrder = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Confirm the order on the customer's devices. Never throws.
    await sendOrderStatusPush(
      userId,
      result.order.id,
