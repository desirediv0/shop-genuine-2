...
import { deleteUserAccount } from "../helper/deleteUserAccount.js";
...
export const registerUser = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email,
      subject: "Your OTP for Email Verification - Shop Genuine",
...
export const forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email,
      subject: "Reset Your Password - Shop Genuine",
...
export const cancelOrder = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (user && user.email) {
      await sendEmail({
        email: user.email,
        subject: `Order #${order.orderNumber} Cancelled - ${process.env.STORE_NAME || "Shop Genuine"}`,
...
export const requestAccountDeletion = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "Confirm Account Deletion - Shop Genuine",
...
export const confirmAccountDeletion = asyncHandler(async (req, res, next) => {
  ...
  // Shared with the in-app flow. A plain user.delete() fails here for anyone who
  // has ever ordered, because Order.userId is RESTRICT — the helper handles that.
...
/**
 * Delete the signed-in user's account in one authenticated call.
 *
 * The emailed-link flow above still exists and is unchanged; this is the path
 * the mobile app uses, because App Store review requires deletion to be
 * completable inside the app rather than bouncing the user to a browser.
 *
 * Re-authentication is required so that a stolen, unlocked phone cannot wipe
 * the account: password users confirm their password, and accounts with no
 * password set (OAuth) confirm by typing DELETE.
 */
export const deleteAccountNow = asyncHandler(async (req, res, next) => {
  ...
  if (user.password) {
    if (!password) {
      throw new ApiError(400, "Please enter your password to confirm deletion");
    }
    ...
    // OAuth account with no password to check against.
  ...
  res
    .status(200)
    .json(new ApiResponsive(200, {}, "Your account has been permanently deleted"));
});
