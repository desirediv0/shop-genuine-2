...
export const getAllCategories = asyncHandler(async (req, res) => {
  ...
  const { page, limit, search, sort, storeVerticalId } = req.query;

  // Build filter conditions
  const where = {
    ...(search ? { name: { contains: search, mode: "insensitive" } } : {}),
    // Store verticals (Genuine Nutrition / Grocery / Pharmacy / Cosmetics) are
    // only linked to products, so a vertical's categories are derived: keep the
    // categories that actually have a visible product in that vertical. This
    // stays correct automatically as products are reassigned, and a category
    // shared by two verticals correctly appears under both.
    ...(storeVerticalId
      ? {
          products: {
            some: {
...
export const getProductsByCategory = asyncHandler(async (req, res) => {
  ...
  // One source of truth for the filter, so the count and the page can never
  // disagree. Narrowed by store vertical when the app has one selected.
  const productWhere = {
    categories: {
      some: {
        category: {
    ...
    isDeleted: false,
    visibility: "PUBLIC",
    ...(storeVerticalId ? { storeVerticalId } : {}),
  };
