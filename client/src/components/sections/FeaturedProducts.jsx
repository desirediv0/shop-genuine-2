...
const SectionHeader = () => (
  <div className="flex items-end justify-between gap-4 mb-7 md:mb-9">
    <div>
      ...
      <p className="text-sm text-stone mt-1.5">Handpicked bestsellers our customers keep reordering</p>
    </div>
    <Link
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        let response = await fetchApi(`/public/products/type/featured?limit=12${storeVerticalQuery}`);

        if (!response?.data?.products?.length) {
          // Fallback to featured products
          response = await fetchApi(`/public/products?featured=true&limit=12${storeVerticalQuery}`);
        }
