    ...
    <AnimatePresence>
      {show && (
        <motion.button
          ...
          className="fixed bottom-6 left-6 z-[55] hidden md:flex h-11 w-11 items-center justify-center border border-gold/60 bg-pink/85 text-gold-light backdrop-blur-sm transition-colors duration-300 hover:bg-pink-dark hover:text-white rounded-full"
...
const LETTERS = "Shop Genuine".split("");
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <div className="flex overflow-hidden">
              {LETTERS.map((ch, i) => (
                <motion.span
                  key={i}
                  className={`font-display text-4xl sm:text-6xl md:text-7xl tracking-[0.03em] ${i < 5 ? "text-gold-light" : "text-azure-light"
                    }`}
                  initial={{ y: "110%" }}
            ...
            <motion.span
              className="mt-5 text-[11px] uppercase tracking-[0.5em] text-white/40"
              initial={{ opacity: 0 }}
              animate={{ opacity: phase === "play" ? 1 : 0 }}
